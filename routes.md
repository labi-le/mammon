...
| `NfsSession.kt` | The contract the provider and the FUSE daemon share: reads (`probeRoot`/`stat`/`list`/`openFile`) plus the mutating half (`createFile`/`makeDirectory`/`remove`/`setAttributes` keyed by parent + name, and `NfsFile.writeAt`), which defaults to refusing so a backend declines by omission rather than by pretending; `supportsWrites` has no default, so every backend answers. `NfsFailure` is the sealed refusal set — `PermissionDenied`, `NotFound`, `AlreadyExists`, `Unsupported`, `Server` — that both frontends map without reading a message. Also `NodeAttrs`/`ChildEntry`, `NfsTarget`, `NFS_READ_CHUNK`/`NFS_WRITE_CHUNK`, the shared directories-first ordering, and `NfsSessions.select` — v4.1 first, v3 on failure, the v4 error kept as a suppressed exception. `NfsFile.readAt` is the positional read FUSE needs; `streamFor` is a default adapter over it. |
| `NfsAccess.kt` | The NFSv3 `NfsSession` over `com.emc.ecs:nfs-client` — stat, list (one READDIRPLUS loop carrying child attributes; symlinks and special files are not listed), positional reads through `Nfs3File.read`. Read-only: `supportsWrites` is false and the mutating half stays at its refusing default. Paths stay export-absolute: `Nfs3File` builds its parent chain by trimming separators and never terminates on a relative path. AUTH_SYS via `CredentialUnix` built from the `AuthIdentity`, matching `NfsV4Access`: under AUTH_NONE a stock Linux server accepts the MOUNT and then refuses GETATTR with NFS3ERR_ACCES. One long-lived session per target; the provider rebuilds it when the config or the identity changes. |
...
- **Writes at the front ends** — the `NfsSession` seam is writable and the NFSv4.1
  backend implements it (create, write, setattr, remove, mkdir), but nothing consumes
  it yet: every mutating FUSE opcode still answers EROFS and the provider is still
  read-only. SAF's writable flags and the FUSE mutating opcodes are separate changes,
  and RENAME is not in the seam at all.
