...
- One exported DocumentsProvider `app.mammon.nfs` (`NfsDocumentsProvider`) guarded by
  ...
  gated per URI by the grant the system issues, `android:permission` already covers reads
  and writes, and adding `android:writePermission` could only restate it or narrow it away
  from DocumentsUI.
